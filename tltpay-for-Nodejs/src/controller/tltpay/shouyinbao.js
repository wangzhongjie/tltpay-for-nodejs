...
class Shouyinbao {
    ...
    async createPayment(options) {
        ...
        let sub_appid=''
        if(options.paytype=='W02') sub_appid=think.config('tltPay').WEIXINGONGZHENGHAOAPPID
        if(options.paytype=='W06') sub_appid=think.config('tltPay').XIAOCHENGXUAPPID
        ...
        _.defaults(options, {
            ...
            notify_url: hostname === 'linkeasy' ? think.config('tltPay').NOTIFYURL_PRO : think.config('tltPay').NOTIFYURL_TEST,
            sub_appid,
        });
    ...
    _getSignature(jsonObject) {
        ...
        for (let k in jsonObject) {
            if (this.isNull(jsonObject[k])) delete jsonObject[k]
        }
